...
public sealed class SubscriptionConfiguration : IEntityTypeConfiguration<Subscription>
{
       ...
       public void Configure(EntityTypeBuilder<Subscription> builder)
       {
              ...
              builder.HasIndex(ca => ca.ConcurrencyStamp)
                     .IsUnique();
              // 1:N Topic -> Subscription (Cascade)
              builder.HasOne(sub => sub.Topic)
                     .WithMany(t => t.Subscriptions)
                     .HasForeignKey(sub => sub.TopicId)
                     .OnDelete(DeleteBehavior.Cascade);
              // 1:N ClientApplication -> Subscription (Cascade)
              builder.HasOne(sub => sub.ClientApplication)
                     .WithMany(ca => ca.Subscriptions)
                     .HasForeignKey(sub => sub.ClientApplicationId)
                     .OnDelete(DeleteBehavior.Cascade);
